...
def recentlysolved():
    ...
    problems_dates = db_session.query(Problem, Solve.time_solved).where(Problem.title==Solve.problem_id).join(Solve, Solve.user_id==session["username"]).group_by(Problem.title).order_by(desc(Solve.time_solved)).limit(6).all()
